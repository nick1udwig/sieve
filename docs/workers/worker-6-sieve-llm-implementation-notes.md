...
Implemented real OpenAI-backed planner and typed guidance adapters under provider abstraction with independent planner/guidance config.
...
- Concrete models:
  ...
  - `OpenAiGuidanceModel`
- Config loading:
  - `SIEVE_PLANNER_*` + `SIEVE_GUIDANCE_*` model/provider/api-base
...
- Structured outputs:
  ...
  - guidance JSON schema for `PlannerGuidanceOutput`
- Typed guidance enforcement:
  - only known `PlannerGuidanceSignal` codes
  - numeric frame validation at decode boundary
...
- Tests:
  ...
  - planner decode path
  - guidance decode path + unknown signal rejection
  - env-gated live guidance smoke test
...
- Optional docs: add planner-specific example alongside guidance example if desired for operator onboarding.
...
- Live paths are gated by:
  ...
  - `OPENAI_API_KEY` (or scoped planner/guidance key vars)
